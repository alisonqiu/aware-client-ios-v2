...
//            contextCard.setWeeklyChart(sensor: sensor, yKeys: ["stationary","walking","running","automotive","cycling"])
    ...
    func addAmbientNoiseCard() {
        if let sensor = AWARESensorManager.shared().getSensor(SENSOR_AMBIENT_NOISE) {
            // TO BE CHANGED TO SPEAKER COUNT
            let speakerCard = ScatterChartCard(frame: CGRect(x:0,y:0, width: self.view.frame.width, height:300))
            speakerCard.setTodaysChart(sensor: sensor, yKeys: ["dnn_res"])
            speakerCard.titleLabel.text = "Ambient Noise | SPEAKER COUNT"
